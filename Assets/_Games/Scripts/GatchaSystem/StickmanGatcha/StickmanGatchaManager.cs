...
public class StickmanGatchaManager 
{
    public const int TIME_WAIT_ADS_FREE = 60;
    [SerializeField] private StickmanGatchaGlobalConfig m_GlobalConfig;
    [SerializeField] private StickmanGatchaData m_GatchaData;
    [SerializeField] private LevelStickmanGatchaData m_LevelGatchaData;
    ...
    #region SaveLoad
    public void LoadData(StickmanGatchaData data)
    {
        m_GlobalConfig = StickmanGatchaGlobalConfig.Instance;
    ...
    public StickmanGatchaData SaveData()
    {
        ...
        LevelStickmanGatchaData levelGatchaData = new LevelStickmanGatchaData
        {
            Level = m_GatchaLevel,
            Point = m_CurrentPoint,
            NextDay = m_NextGatchaDay.ToString(CultureInfo.InvariantCulture),
        };
        StickmanGatchaData cardGatchaData = new StickmanGatchaData(levelGatchaData, gatchaButtonData);
    ...
    public List<StickmanConfig> GoGatcha(int num, GatchaButtonType btnType)
    {
        Debug.Log("GatchaStickman");
        List<StickmanConfig> list = new();
        for (int i = 0; i < num; i++)
        {
            Rarity rarity = m_GlobalConfig.levelConfig[m_GatchaLevel - 1].rarityConfig.GetRandomItem();
            Debug.Log(rarity);
            StickmanConfig stickmanConfig = StickManManager.Ins.GetRandomStickmanConfigWithRarity(rarity);
            list.Add(stickmanConfig);
            UpgradeGatcha();
    ...
    private void AddGatchaData(List<StickmanConfig> list) 
    {
         StickManManager.Ins.AddStickmanPiece(list,1);
    }
    ...
    public StickmanGatchaLevelConfig GetCurrentLevelConfig()
    {
...
[System.Serializable]
public class StickmanGatchaData
{
    public LevelStickmanGatchaData lD;
    public GatchaButtonData bD;

    public LevelStickmanGatchaData LevelGatchaData { get => lD; set => lD = value; }
    public GatchaButtonData ButtonData { get => bD; set => bD = value; }
    public StickmanGatchaData()
    {
        lD = new LevelStickmanGatchaData();
        bD = new GatchaButtonData();
    }
    public StickmanGatchaData(LevelStickmanGatchaData lv, GatchaButtonData btn)
    {
        LevelGatchaData = lv;
        ButtonData = btn;
    }
...
[System.Serializable]
public class LevelStickmanGatchaData
{
    public int lv;
    public int pt;
    public string nD;
    ...
    public LevelStickmanGatchaData()
    {
        lv = 1;
        pt = 0;

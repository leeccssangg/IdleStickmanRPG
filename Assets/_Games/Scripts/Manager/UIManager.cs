...
public class UIManager :SingletonFreeAlive<UIManager>
{
    ...
    public T GetUI<T>() where T : UICanvas
    {
        if (!IsLoaded<T>())
        {
            UICanvas cv = GetUIPrefab<T>();
            Transform parent = cv.IsExpand ? CanvasParentTF2 : CanvasParentTF;
            UICanvas canvas = Instantiate(cv,parent);
    ...
    public void SetActiveMaintab(bool isActive)
    {
        GetUI<UIPanelMainButton>().gameObject.SetActive(isActive);
    }
    public void PlayEarnResourceEffect(ResourceData.ResourceType type,Vector3 pos)
    {
        OpenUI<UIPanelEarnResource>().PlayEarnResourceEffect(pos,type);
    }
}
